    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.trips.store") }}" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <select class="form-control select2 {{ $errors->has('city_from') ? 'is-invalid' : '' }}" name="city_from_id" id="city_from_id" required>
                    @foreach($city_froms as $id => $city_from)
                        <option value="{{ $id }}" {{ old('city_from_id') == $id ? 'selected' : '' }}>{{ $city_from }}</option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <select class="form-control select2 {{ $errors->has('city_to') ? 'is-invalid' : '' }}" name="city_to_id" id="city_to_id" required>
                    @foreach($city_tos as $id => $city_to)
                        <option value="{{ $id }}" {{ old('city_to_id') == $id ? 'selected' : '' }}>{{ $city_to }}</option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <input class="form-control {{ $errors->has('adults') ? 'is-invalid' : '' }}" type="number" name="adults" id="adults" value="{{ old('adults', '1') }}" step="1" required>
            ...
            <div class="mb-3">
                ...
                <input class="form-control {{ $errors->has('children') ? 'is-invalid' : '' }}" type="number" name="children" id="children" value="{{ old('children', '0') }}" step="1">
